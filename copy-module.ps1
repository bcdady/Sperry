...
{
$moduleDestination = Join-Path -Path $moduleDestination -ChildPath $dirName;
$Error
$Error.Clear()
}

write-output  -InputObject "`nCopying module to $moduleDestination `n";

try
{
    Get-ChildItem $PSScriptRoot -Recurse -Exclude 'copy-module.ps1','.git*','*.md' | 
    Copy-Item -Destination $moduleDestination ;
}
catch
{
    Write-Warning "Fatal error: Unable to copy module to new destination directory: $moduleDestination"
}
finally
{
$Error
...
try
{
    if (Test-Path (Join-Path -Path $moduleDestination -ChildPath "$dirName.psd1")) {
        write-host "`n";
